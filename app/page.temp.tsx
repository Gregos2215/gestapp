      ...
      <div className="min-h-screen bg-gray-50">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (profileEdits.firstName) userUpdates.firstName = profileEdits.firstName;
                            if (profileEdits.lastName) userUpdates.lastName = profileEdits.lastName;
                            
                            if (Object.keys(userUpdates).length > 0) {
                              await updateDoc(doc(db, 'users', customUser.uid), userUpdates);
                            }
                            
                            // Si c'est un employeur et que les titres ont été modifiés, mettre à jour les paramètres du centre
                            if (customUser.isEmployer && customUser.centerCode && typeof customUser.centerCode === 'string' && customUser.centerCode.trim() !== '') {
                              try {
                                const centerRef = doc(db, 'centers', customUser.centerCode);
                                
                                // Vérifier si le document du centre existe déjà
                                const centerDoc = await getDoc(centerRef);
                                ...
                                if (centerDoc.exists()) {
                                  ...
                                  await setDoc(centerRef, {
                                    ...
                                    createdAt: new Date(),
                                    updatedAt: new Date(),
                                    createdBy: customUser.uid,
                                    updatedBy: customUser.uid
                                  });
                                }
                              } catch (error) {
                                console.error('Error updating center document:', error);
                            ...
                            setProfileEdits({ firstName: '', lastName: '' });
                          } catch (error) {
        ...
        <div className="fixed top-0 left-0 z-50 w-full bg-white border-b border-gray-200 lg:hidden">
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="p-2.5 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200 flex items-center space-x-2"
            >
              {isMobileMenuOpen ? (
                <>
                  <MenuCloseIcon className="h-6 w-6" aria-hidden="true" />
